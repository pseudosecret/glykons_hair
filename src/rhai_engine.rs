        ...
        other => {
            if let Ok(degree) = other.parse::<i32>() {
                if degree >= 0 && degree <= 7 {
                    ...
                    let note_idx = (degree % 7) as usize;
                    scale[note_idx] + (octaves * 12)
                } else {
...
pub fn evaluate_rhai(script: &str) -> Result<String, String> {
    ...
    engine.register_type_with_name::<StrudelPattern>("Pattern")
        .register_fn("note", |mut p: StrudelPattern, n: &str| -> StrudelPattern { p.notes_raw = n.to_string(); p })
        .register_fn("s", |mut p: StrudelPattern, s: &str| -> StrudelPattern { p.synth = s.to_string(); p })
        .register_fn("sound", |mut p: StrudelPattern, s: &str| -> StrudelPattern { p.synth = s.to_string(); p })
        .register_fn("fast", |mut p: StrudelPattern, mult: Dynamic| -> StrudelPattern { 
            if let Ok(v) = mult.as_float() { p.speed *= v as f32; }
            else if let Ok(v) = mult.as_int() { p.speed *= v as f32; }
            p 
        })
        .register_fn("slow", |mut p: StrudelPattern, div: Dynamic| -> StrudelPattern { 
            if let Ok(v) = div.as_float() { p.speed /= v as f32; }
            else if let Ok(v) = div.as_int() { p.speed /= v as f32; }
            p 
        })
        .register_fn("rev", |mut p: StrudelPattern| -> StrudelPattern { p.is_rev = !p.is_rev; p })
        .register_fn("lpf", |mut p: StrudelPattern, freq: Dynamic| -> StrudelPattern { p.effects.push(format!("lpf {} 1.0", freq)); p })
        .register_fn("hpf", |mut p: StrudelPattern, freq: Dynamic| -> StrudelPattern { p.effects.push(format!("hpf {} 1.0", freq)); p })
        .register_fn("gain", |mut p: StrudelPattern, gain: Dynamic| -> StrudelPattern { p.effects.push(format!("mul {}", gain)); p })
